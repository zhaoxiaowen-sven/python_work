...
def gen_chart_style2(chart, d, sheet_name, x_name, y_name):
    i = len(d)
    l = list(d.keys())
...
def gen_chart_style(chart, i, sheet_name, x_name, y_name):
    chart.add_series({
...
def gen_chart_style3(chart, sort_list, sheet_name, x_name, y_name):
    i = len(sort_list)
    # l = list(d.keys())
...
def make_proc_spread_sheet(wb, sheet_name, sort_list, keys):
...
def method_spread(list_arr, sort_list):
    len1 = len(list_arr)
    spread_dict = {}
    for v in sort_list:
        process = v[0]
...
def make_screen_focused_sheet(wb, sort_list, sheet_name):
...
def make_screen_onoff_sheet(wb, sort_list, sheet_name):
...
def method_spread2(list_arr, sort_list):
    # print(sort_list)
    len1 = len(list_arr)
    spread_dict = {}
    for v in sort_list:
...
def make_kill_sheets(kill_result, xlsx_name):
    # print(kill_result)
...
def make_mem_sheets(mem_result, xlsx_name):
...
def __make_battery_sheets(battery, xlsx_name):
...
def make_launch_sheets(results):
...
def make_app_use_time_sheet(results):
...
def make_next_app_sheet(results):
    # 应用的跳转关系
    # 移动一次计算
...
class SheetBuilder(object):
    ...
    def make_pss_sheets(self, pss_list, xlsx_name):
        ...
        spread_dict = method_spread2(list_arr, pss)
        make_proc_spread_sheet(wb, "pss_spread_sheet", spread_dict, generate_unit(list_arr))
        wb.close()
        # pass
    ...
    def make_screen_sheets(self, results, xlsx_name):
        ...
        make_screen_onoff_sheet(wb, v, "screen_onoff")
        v = results.get("screen_focused")
        make_screen_focused_sheet(wb, v, "screen_focused")
        wb.close()
    ...
    def make_proc_sheets(self, results, xlsx_name):
        ...
        spread_dict = method_spread(list_arr, sort_list)
        # print(spread_dict)
        make_proc_spread_sheet(wb, "spread_sheet", spread_dict, generate_unit(list_arr))
    ...
    def make_proc_sub_sheet(self, wb, sheet_sub_name, sort_list):
        ...
        gen_chart_style(chart, i, sheet_sub_name, "Activity", "Times")
    ...
    def make_proc_main_sheet(self, wb, sort_list, fmt):
        ...
        chart = wb.add_chart({'type': 'line'})
        sheet.insert_chart(i + 1, 0, chart)
        gen_chart_style3(chart, sort_list, "proc_main", "times", "Time(/ms)")
    ...
    def make_except_sheet(self, sort_list, wb, sheet_name):
        ...
        chart = wb.add_chart({'type': 'line'})
        sheet.insert_chart(i + 1, 0, chart)
        gen_chart_style(chart, i, sheet_name, "PkgName", "Times")
    ...
    def make_app_spread_sheet(self, wb, sorted_tuple, sheet_name, fmt):
        ...
        gen_chart_style2(chart, tmp_dict, sheet_name, "Time", "Times")
    ...
    def make_main_sheet(self, sheet_name, sorted_tuple, wb, fmt):
        ...
        chart = wb.add_chart({'type': 'line'})
        sheet.insert_chart(i + 1, 0, chart)
        gen_chart_style(chart, i, sheet_name, "Packages", "Times")
